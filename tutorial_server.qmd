...
1A. Navigate to the folder containing the file(s) you want to upload
...
1B. Navigate to the folder containing the folder you want to upload using `cd`
...
Your prompt at the shell should now have switched to `sftp>` and although it is the same window your are now looking at the files on the server! Try it by typing the `ls` command at the prompts. This will list all the files and folders within the `shares` folder on Aurora.
...
5. ou can also upload a local file to the server by using the `put` sFTP command
```{bash, eval = F}
put [file_name_local]
```
